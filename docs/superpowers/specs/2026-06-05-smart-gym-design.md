...
- 数据库：使用 PostgreSQL。
- 文件存储：使用后端本地存储，文件保存在后端目录下。
- 前端组织：用户端和管理端放在同一个 React 应用中，通过路由隔离。
- AI 配置：提供 AI Provider 配置页面，并且每个用户的 AI 配置相互隔离。
...
flowchart LR
  ...
  API --> Storage["后端本地文件存储<br/>教程媒体、食物图片"]
```
...
- 每个用户的 AI Provider 配置独立保存，普通用户只能读取和修改自己的 AI 配置。
...
- `ai_provider_configs`：所属用户、Provider 类型、Base URL、模型名称、密钥引用、启用状态。
...
AI 调用必须基于当前登录用户读取对应的 `ai_provider_configs`。系统不能把某个用户的模型配置、Base URL 或密钥用于其他用户请求。
...
- 榜单：周榜、月榜。
- 我的：身体数据、目标、偏好、AI Provider 配置、设置。
...
- 数据库使用 PostgreSQL。
- 文件存储使用后端本地目录，后续如果需要再迁移到对象存储。
- 用户端和管理端使用同一个 React 应用，通过路由和权限隔离。
- AI Provider 必须提供配置页面。
- AI Provider 配置按用户隔离，不能共享或串用其他用户配置。
